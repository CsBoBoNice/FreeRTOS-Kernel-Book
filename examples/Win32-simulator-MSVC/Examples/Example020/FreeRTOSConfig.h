/*
    ...
    ***************************************************************************
     *                                                                       *
     *    FreeRTOS提供完全免费但专业开发的、强大的、严格质量控制的、         *
    ...
    ***************************************************************************
    ...
    FreeRTOS是自由软件；您可以根据由自由软件基金会发布的GNU通用公共许可证
    (版本2) >>!并经FreeRTOS例外修改!<< 进行重新分发和/或修改。

    >>! 注意：对GPL的修改是为了允许您分发包含FreeRTOS的组合作品，而不必提供
    >>! FreeRTOS内核之外的专有组件的源代码。

    FreeRTOS的分发希望它会有用，但不提供任何担保；甚至没有对适销性或特定
    用途适用性的暗示担保。完整的许可证文本可从以下链接获得：
    ...
    ***************************************************************************
     *                                                                       *
     *    遇到问题？请先阅读FAQ"我的应用程序无法运行，可能是什么问题？"      *
     *                                                                       *
    ...
    ***************************************************************************
    ...
    （不可或缺的生产力工具）、兼容DOS的FAT文件系统以及我们的小型线程感知UDP/IP栈。

    http://www.OpenRTOS.com - Real Time Engineers ltd将FreeRTOS授权给High 
    Integrity Systems以OpenRTOS品牌销售。低成本OpenRTOS许可提供票务支持、
    赔偿和中间件。
...
#define configMAX_TASK_NAME_LEN					( 12 )  /* 任务名称最大长度为12字符 */
#define configUSE_TRACE_FACILITY				0  /* 不使用跟踪功能 */
#define configUSE_16_BIT_TICKS					0  /* 使用32位滴答计数 */
...
#define configMAX_CO_ROUTINE_PRIORITIES 		2  /* 协程最大优先级为2 */
