/*
    ...
    ***************************************************************************
     ...
     *                                                                       *
     *    通过从以下网址购买FreeRTOS教程书籍、参考手册或两者都购买         *
     *    来帮助自己快速入门并支持FreeRTOS项目：                            *
    ...
    ***************************************************************************
    ...
    重新分发和/或修改它，并由FreeRTOS例外>>!并由!<<FreeRTOS例外修改。

    >>! 注意：对GPL的修改是为了允许您分发包含FreeRTOS的组合作品，而不必提供
    >>! FreeRTOS内核之外的专有组件的源代码。

    FreeRTOS的发布希望它能有所帮助，但不提供任何保证；甚至没有对适销性或特定
    ...
    ***************************************************************************
    ...
    一个不可或缺的生产力工具、与DOS兼容的FAT文件系统和我们的小型线程感知UDP/IP堆栈。

    http://www.OpenRTOS.com - Real Time Engineers ltd将FreeRTOS许可给High Integrity Systems，
    以便在OpenRTOS品牌下销售。低成本的OpenRTOS许可提供有票据支持、赔偿和中间件。

    http://www.SafeRTOS.com - High Integrity Systems还提供经安全工程处理且独立SIL3认证的版本，
 ...
 *
 * 这些参数在FreeRTOS.org网站上可获得的FreeRTOS API文档的
 * '配置'部分和FreeRTOS参考手册中有详细描述。
 *----------------------------------------------------------*/

#define configUSE_PREEMPTION					1       /* 启用抢占式调度器 */
#define configUSE_PORT_OPTIMISED_TASK_SELECTION	1       /* 启用优化的任务选择方法 */
#define configMAX_PRIORITIES					5       /* 设置最大任务优先级为5 */
...
#define configMINIMAL_STACK_SIZE				( ( unsigned short ) 50 ) /* 最小堆栈大小为50，在模拟环境中堆栈只需容纳一个小结构，因为实际堆栈是win32线程的一部分 */
#define configTOTAL_HEAP_SIZE					( ( size_t ) ( 20 * 1024 ) ) /* 总堆内存大小为20KB */
...
#define configIDLE_SHOULD_YIELD					1       /* 空闲任务应在有同优先级任务就绪时让出CPU */
...
#define configTIMER_TASK_PRIORITY				( configMAX_PRIORITIES - 1 ) /* 定时器服务任务优先级为最高优先级减1 */
#define configTIMER_QUEUE_LENGTH				20      /* 定时器命令队列长度 */
#define configTIMER_TASK_STACK_DEPTH			( configMINIMAL_STACK_SIZE * 2 ) /* 定时器任务堆栈大小为最小堆栈的2倍 */
...
/* 此演示不使用一个或多个示例统计格式化函数，这些函数将由uxTaskGetSystemState()函数提供的原始数据格式化为人类可读的ASCII形式 */
#define configUSE_STATS_FORMATTING_FUNCTIONS	0       /* 不使用统计格式化函数 */

/* 将以下定义设置为1以包含API函数，或设置为0以排除API函数。在大多数情况下，链接器会自动移除未使用的函数。 */
...
#define INCLUDE_vTaskDelayUntil					1       /* 包含任务精确延时函数 */
...
#define INCLUDE_xTimerPendFunctionCall			1       /* 包含定时器挂起函数调用函数 */
